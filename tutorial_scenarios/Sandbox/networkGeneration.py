...
class experimentConfiguration:
    ...
    def __init__(self, lconf):
        ...
        self.func_REQUESTPROB = "1.0"  # App's popularity. This value define the probability of source request an application
    ...
    def loadConfiguration(self, lmyConfiguration):
        # Configuration for the IEEE IoT journal experiments
        if lmyConfiguration == 'small':
            # Cloud
    ...
    def getPlacementMatrix(self):
        ...
        self.memNodes = list(self.nodeFreeResources.values())
        self.memSer = list(self.servicesResources.values())
        ...
        for a in range(self.numApplications):
            for r in range(self.numRequests):
                for s in range(self.numServices):
                    mdl.add_constraint(self.requestPerApp[a, r] * mdl.sum(placementMatrix[(a, r), (s, n)] for n in range(self.numNodes)) <= 1, ctname='requestMostOneServer_app%d_req%d_ser%d' % (a, r, s))
